...
def update_t(h_x, h_y, t_x, t_y):
    ...
    return sum_two_lists(t_pos, how_to_move)
...
for [direction, dist] in directions:
    ...
    for each_step in range(int(float(dist))):
        h_pos = sum_two_lists(h_pos, dir_update)
        touching = h_t_touching(h_pos[1], h_pos[0], t_pos[1], t_pos[0])
        #print("touching?", touching)
        if touching == False:
            t_pos = update_t(h_pos[1], h_pos[0], t_pos[1], t_pos[0])
            visited_t_pos.add(tuple(t_pos))
        #print("h_pos",h_pos)
        #print("t_pos",t_pos)
    #print()
